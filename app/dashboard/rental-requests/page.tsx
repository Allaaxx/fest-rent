...
export default function RentalRequestsPage() {
  ...
  useEffect(() => {
    const fetchRentals = async () => {
      try {
        ...
        setError(error instanceof Error ? error.message : "Failed to load rental requests")
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleApprove = async (rentalId: string) => {
    ...
    try {
      ...
      setRentals(rentals.map((r) => (r.id === rentalId ? { ...r, status: "approved" } : r)))
    } catch (error) {
  ...
  const handleReject = async (rentalId: string) => {
    ...
    try {
      ...
      setRentals(rentals.map((r) => (r.id === rentalId ? { ...r, status: "rejected" } : r)))
    } catch (error) {
      ...
      <main className="mx-auto max-w-4xl px-6 py-8">
        <div className="mb-8">
          ...
          <p className="text-slate-400 mt-2">
            {pendingRentals.length} pending request{pendingRentals.length !== 1 ? "s" : ""}
          </p>
        </div>
              ...
              <Card key={rental.id} className="bg-slate-800 border-slate-700">
                <CardContent className="pt-6">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <p className="text-slate-300 mb-2">
                        <span className="font-semibold">Period:</span> {rental.start_date} to {rental.end_date}
                      </p>
                      <p className="text-2xl font-bold text-blue-400">${rental.total_value.toFixed(2)}</p>
                    </div>
                  ...
                  <div className="flex gap-2 mt-4">
                    ...
                    >
                      {actionLoading === rental.id ? "Processing..." : "Approve"}
                    </Button>
                    <Button
